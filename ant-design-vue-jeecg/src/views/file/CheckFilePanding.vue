    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="updateFileStatus(record, '1')">通过</a>
          <a-divider type="vertical" />
          <a @click="updateFileStatus(record, '2')">否决</a>
          <a-divider type="vertical" />
          <a-dropdown>
...
export default {
  ...
  created() {
    ...
    debugger
    this.setQueryParams['fileStatus'] = '0'
  },
  computed: {
  ...
  methods: {
    ...
    updateFileStatus(record, status) {
      console.log('fileStatus', status)
      record.fileStatus = status
      putAction(`/UploadRecord/uploadRecord/edit`, record)
        .then((res) => {
          if (res.success) {
